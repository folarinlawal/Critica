...
// ── Thread component: conversation anchored to a single finding ──
function FindingThread({ finding, type, productType, personaDesc, constraintsSummary }) {
...
export default function App() {
  ...
  const getPersonaDesc = () => {
    ...
    return selectedPersona === "custom"
      ? customPersona
      : `${persona?.label} — ${persona?.description}. Traits: ${persona?.traits}`;
  };
...
{"narrativeWalkthrough":"1-2 sentences as persona","overallScore":7,"actNow":[{"severity":"high","title":"title","description":"one sentence","recommendation":"one sentence","effort":"low"}],"roadmap":[{"severity":"high","title":"title","description":"one sentence","recommendation":"one sentence","userImpact":"one sentence","pmNote":"one sentence"}],"strengths":["one sentence"],"priorityFocus":"one sentence"}`;
  ...
  const generatePMSummary = () => {
    ...
    return [
      `DESIGN REVIEW — ${productType.toUpperCase()}`,
      `Persona Tested: ${personaLabel}`,
      `UX Score: ${critique.overallScore}/10`,
      ``,
      `USER EXPERIENCE SUMMARY`,
      critique.narrativeWalkthrough,
      ``,
      `ROADMAP OPPORTUNITIES (${critique.roadmap?.length || 0} items)`,
      `Issues identified during design review that fall outside current sprint constraints.`,
      ``,
      ...(critique.roadmap || []).map((item, i) => [
      ...
      ...(critique.actNow || []).map((item, i) => `${i + 1}. ${item.title} — ${item.recommendation}`),
      ``,
  ...
  const reset = () => {
    setStep("input"); setCritique(null); setFiles([]);
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
            ...
            <div>
              <div style={{ display: "flex", gap: 4, marginBottom: 20, background: "rgba(255,255,255,0.03)", borderRadius: 10, padding: 4 }}>
                {[
                  { id: "now", label: "Act Now", count: critique.actNow?.length || 0, color: "#34C759" },
                  { id: "roadmap", label: "Roadmap", count: critique.roadmap?.length || 0, color: "#FF9500" },
                  { id: "pm", label: "PM Export", count: null, color: "#6366F1" }
                ].map(tab => (
              ...
              {activeTab === "now" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ fontSize: 12, color: "#4B5563", lineHeight: 1.5 }}>
                    Click <strong style={{ color: "#6B7280" }}>Discuss this finding</strong> on any card to push back, add context, or ask for alternatives.
                  </div>
                  {(critique.actNow || []).length === 0
                    ...
                    : (critique.actNow || []).map((fp, i) => (
                      <FindingThread key={i} finding={fp} type="now" productType={productType} personaDesc={personaDesc} constraintsSummary={constraintsSummary} />
                    ))
                  }
                </div>
              )}
              ...
              {activeTab === "roadmap" && (
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div style={{ padding: "12px 16px", borderRadius: 10, background: "rgba(255,149,0,0.06)", border: "1px solid rgba(255,149,0,0.15)", fontSize: 12, color: "#9CA3AF", lineHeight: 1.6 }}>
                    Outside your current constraints. Use <strong style={{ color: "#E8E8F0" }}>PM Export</strong> to share these. Discuss any finding to explore workarounds.
                  </div>
                  {(critique.roadmap || []).length === 0
                    ...
                    : (critique.roadmap || []).map((fp, i) => (
                      <FindingThread key={i} finding={fp} type="roadmap" productType={productType} personaDesc={personaDesc} constraintsSummary={constraintsSummary} />
                    ))
                  }
                </div>
              )}
